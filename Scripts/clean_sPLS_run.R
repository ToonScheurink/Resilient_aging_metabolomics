...
spls_final <- spls(X_sub, Y, ncomp = 1, keepX = 3080) 
...
ggsave("sPLS_cogres.svg", plot = sPLS_cogres, dpi =300, width = 10, height = 7)
...
cv_selected <- cv_spls(X_sub, Y, folds, ncomp = 1, keepX = rep(3080, 1))
...
for (p in 1:n_perm) {
  ...
  cv_perm <- cv_spls(X_sub, Y_perm, folds, ncomp = 1, keepX = rep(3080, 1))
...
### Extracting top 3080, rank and weight
...
###Visualizing and stats for internal stability 
# Median
median_stab <- median(df_loadings_full$Stability, na.rm = TRUE)
# Range (min and max)
range_stab <- range(df_loadings_full$Stability, na.rm = TRUE)
# Percentage of features selected in >70% of runs
pct_over70 <- mean(df_loadings_full$Stability >= 0.70) * 100

median_stab
range_stab
pct_over70

# Add absolute weight column
df_loadings_full$AbsWeight <- abs(df_loadings_full$Weight)
...
hexplot_with_stats <- ggplot(df_loadings_full, aes(x = Weight, y = Stability)) +
  ...
  # --- Clean theme: axis lines + ticks ---
  theme_minimal(base_size = 14) +
  theme(
    panel.grid = element_blank(),     
    axis.line = element_line(),       
    axis.ticks = element_line(),       # <-- add ticks
    axis.ticks.length = unit(3, "pt"), # <-- tick length
...
ggsave("hexplot_with_stats.svg", plot = hexplot_with_stats, dpi =300, width = 10, height = 7)
...
spls_ext <- spls(X_ext_aligned, y_true, ncomp = 1, keepX = 3080)
...
ggsave("feature_stability_ext.svg", plot = feature_stability_ext, dpi =300, width = 12, height = 5)
