...
###adding metabolite info
df_combined <- df_clean %>%
  left_join(
    model_metab %>% select(Scan, Direction),
    by = "Scan")
...
### Counts per class
df_count_class <- df_combined %>%
  group_by(Scan, Direction, class) %>%
  summarise(count = n(), .groups = "drop")

df_count_class_for_wide <- df_combined %>%
  group_by(Scan, class) %>%
  summarise(count = n(), .groups = "drop")
...
df_class_direction <- df_combined %>%
...
#ggsave("microbes_counts_class_2.svg", plot = microbes_counts_class_2, height = 4, width = 8, dpi = 300)
